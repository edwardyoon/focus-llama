...
- The mask only applies to global-attention layers. Sliding-window and recurrent (e.g. Gated DeltaNet) layers are untouched, so hybrid models save less, and exact `seq_rm` equivalence does not hold for recurrent state.
